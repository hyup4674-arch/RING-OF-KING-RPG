...
def init_character_stats(job_type):
    ...
    base_stats = {
        ...
        "con": 10,
        "int": 10,
        "equipped_weapon": weapon_shop[0],
        "equipped_armor": armor_shop[0],
        "inventory_weapons": [weapon_shop[0]],
        "inventory_armors": [armor_shop[0]],
        "learned_magic": [magic_guild[0]],
        "learned_skills": [combat_dojo[0]],
...
def check_level_up(player):
    ...
    while player["exp"] >= player["max_exp"]:
        ...
        player["stat_points"] = player.get("stat_points", 0) + 3
        leveled_up = True
        level_logs.append(
...
def process_auto_combat():
    ...
    if best_action and best_action[0] == "magic":
        ...
        if mg.get("type") == "frost":
            ...
            combat_log.append({
                "text": f"❄️ 냉기 마법 [{mg['name']}] 시전! 적이 동결 상태가 됩니다.",
                "type": "player",
            })
        elif mg.get("type") == "fire":
            enemy["burn_status"] = True
            combat_log.append({
                "text": f"🔥 불 마법 [{mg['name']}] 시전! 적이 화상 상태가 됩니다.",
                "type": "player",
            })
...
if st.session_state.game_mode == "CLASS_SELECT":
    ...
    with side_col:
        ...
        st.write(f"**직업**: {p['job']}")
        st.write(
            f"📈 **레벨**: {p['level']} (EXP: {p['exp']}/{p['max_exp']})"
        )
    ...
    with main_col:
        if st.session_state.game_mode == "COMBAT":
            ...
            def handle_user_action(action_text, difficulty=None):
                ...
                prompt_text = (
                    f"{action_text} (선택한 경로 난이도: {difficulty})"
                    if difficulty
                    else action_text
                )

                with st.spinner(
                    "AI 게임 마스터가 다음 이야기를 생성하고 있습니다... 🎲"
                ):
            ...
            if "choices" in last_turn and last_turn["choices"]:
                ...
                for idx, col in enumerate([c1, c2, c3]):
                    if idx < len(choices):
                        if col.button(
                            f"방향 {idx+1}: {choices[idx]}",
                            key=f"btn_{current_turn}_{idx}",
                        ):
                            difficulties = [
                                "하 (쉬움)",
                                "중 (보통)",
                            ...
                            handle_user_action(
                                choices[idx], difficulty=difficulties[idx]
                            )

            user_input = st.chat_input(
                "원하는 행동을 직접 입력하세요 (예: 횃불을 켜고 조심스럽게"
                " 전진한다)"
            )
            if user_input:
