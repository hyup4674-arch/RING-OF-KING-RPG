...
# 🏪 [상점 리뉴얼 함수 (스탯 맞춤형 입고)]
def renew_shops():
    stats_ref = st.session_state.stats
    lvl = stats_ref["level"]
    s_val = stats_ref["str"]
    c_val = stats_ref["con"]
    i_val = stats_ref["int"]

    w_names = ["강철 검", "미스릴 단검", "룬 블레이드", "왕실 장검", "엘프의 명검"]
    a_names = ["철갑옷", "기사단 갑옷", "미스릴 흉갑", "용비늘 갑옷"]
    m_names = ["라이트닝 볼트", "블리자드", "아케인 셰어", "프로스트 노바"]
    s_names = ["회전 베기", "파쇄격", "질풍 刺", "급소 찌르기"]

    st.session_state.weapon_shop = [
        {
            "name": f"{random.choice(w_names)} +{lvl}",
            "damage": int((s_val + lvl * 3) * 1.2),
            "required_str": max(5, s_val - 2 + random.randint(0, 3)),
            "price": int((s_val + lvl * 3) * 10),
        },
        {
            "name": f"{random.choice(w_names)} Mk.{lvl}",
            "damage": int((s_val + lvl * 4) * 1.4),
            "required_str": max(5, s_val + random.randint(1, 5)),
            "price": int((s_val + lvl * 4) * 15),
        },
    ]

    st.session_state.armor_shop = [
        {
            "name": f"{random.choice(a_names)} Lv.{lvl}",
            "defense": int((c_val + lvl * 2) * 0.9),
            "required_con": max(5, c_val - 2 + random.randint(0, 3)),
            "price": int((c_val + lvl * 2) * 10),
        },
        {
            "name": f"{random.choice(a_names)} 강화형",
            "defense": int((c_val + lvl * 3) * 1.1),
            "required_con": max(5, c_val + random.randint(1, 5)),
            "price": int((c_val + lvl * 3) * 14),
        },
    ]

    st.session_state.magic_guild = [
        {
            "name": random.choice(m_names),
            "damage": int((i_val + lvl * 4) * 1.5),
            "mp_cost": 15 + lvl * 2,
            "price": 300 + lvl * 80,
        }
    ]

    st.session_state.combat_dojo = [
        {
            "name": random.choice(s_names),
            "damage": int((s_val + lvl * 3) * 1.3),
            "mp_cost": 10 + lvl * 2,
        }
    ]
    save_game()
...
with right_col:
    ...
    # 🔄 [상점 메뉴 리뉴얼 버튼 추가]
    st.markdown("---")
    if st.button("🔄 상점 메뉴 리뉴얼", use_container_width=True):
        renew_shops()
        st.success("현재 스탯에 맞게 상점 품목이 갱신되었습니다!")
        st.rerun()

    # ⚡ [스탯 3포인트씩 분배 버튼]
    if stats.get("stat_points", 0) >= 3:
        st.markdown("---")
        st.success(f"잔여 스탯: {stats['stat_points']} P")
        col_s1, col_s2 = st.columns(2)
        if col_s1.button("💪 힘+3", use_container_width=True, key="stat_str"):
            stats["str"] += 3
            stats["stat_points"] -= 3
            save_game()
            st.rerun()
        if col_s2.button("⚡ 민첩+3", use_container_width=True, key="stat_agi"):
            stats["agi"] += 3
            stats["stat_points"] -= 3
            save_game()
            st.rerun()
        col_s3, col_s4 = st.columns(2)
        if col_s3.button("❤️ 체력+3", use_container_width=True, key="stat_con"):
            stats["con"] += 3
            stats["max_hp"] += 9
            stats["hp"] = stats["max_hp"]
            stats["stat_points"] -= 3
            save_game()
            st.rerun()
        if col_s4.button("🧠 지능+3", use_container_width=True, key="stat_int"):
            stats["int"] += 3
            stats["max_mp"] += 6
            stats["mp"] = stats["max_mp"]
            stats["stat_points"] -= 3
    ...
    if not api_key_input:
        ...
        if st.session_state.game_mode == "COMBAT":
            ...
            if enemy["hp"] <= 0:
                ...
                log += f"\n🎉 승리! 보상 획득 ({gold_rew}G){' [레벨 업! (스탯+3)]' if leveled else ''}"
                st.session_state.history.append({
                    "role": "assistant",
                    "narrative": log,
